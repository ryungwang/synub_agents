...
이 문서는 `synub_agents` 운영자가 직접 확인해야 하는 작업만 정리한다. 코드 수정, 빌드, 커밋, 문서 갱신은 에이전트가 처리할 수 있지만 토큰 발급, 운영 방식 결정, 직원 PC 배포 검수처럼 실제 권한과 회사 정책이 필요한 일은 운영자가 확인해야 한다.
