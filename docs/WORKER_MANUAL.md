...
이 worker는 운영 콘솔의 중앙 작업 요청을 처리하는 중앙 실행기다. 직원 앱에서 직원이 직접 여는 로컬 프로젝트 탭과 프로젝트별 AI 팀은 이 worker가 처리하지 않는다.
...
| 구분 | 중앙 Codex worker | 직원 앱 로컬 AI 팀 |
| --- | --- | --- |
| 실행 위치 | 운영자가 설정한 worker 머신 | 직원이 실행한 데스크톱 앱 |
| 작업 경로 | `CODEX_WORKSPACE_ROOT` 또는 중앙 프로젝트 `workspacePath` | 직원이 앱에서 연 로컬 프로젝트 폴더 |
| 시작 방식 | 운영 콘솔에서 작업 검토 후 실행 생성 | 프로젝트 탭에서 AI 팀 구성 후 시작 |
| 결과 | diff, 로그, PR 링크를 운영 콘솔/API에 보고 | 프로젝트 탭의 팀 상세, 메시지, 작업 보드에서 확인 |
| 용도 | 운영 검토가 필요한 중앙 요청 처리 | 직원의 프로젝트별 로컬 작업, 검토, 실험 |

직원이 로컬 프로젝트 안에서 AI 작업을 하려면 worker를 기다리는 것이 아니라 `Synub Teams AI` 앱에서 해당 프로젝트 폴더를 열고 AI 팀을 구성해야 한다.
